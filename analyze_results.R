library(lubridate)
offsite_dir <- .get_config_path("LOCAL_SEQUENTIAL_TEST_DATA_DIR")
result_dir <- paste0(offsite_dir, "results/")

# slow res
fs <- list.files(result_dir, pattern = "^classical_res_[0-9]+\\.rds$")
# ensure all files present
f_names <- sort(fs)

# load the files and process the results
res_list <- lapply(paste0(result_dir, f_names), FUN = function(f) readRDS(f))
result_df <- lapply(res_list, FUN = function(r) r$finite_sample_classical_res) |>
  data.table::rbindlist()
result_df$rejected <- NULL
if ("hyp_idx" %in% names(result_df)) result_df$hyp_idx <- NULL
result_df$rejected <- p.adjust(p = result_df$p_value, method = "BH") < 0.1
total_s <- sapply(res_list, FUN = function(r) r$finite_sample_classical_time[["elapsed"]]) |> sum()
seconds_to_period(total_s)
classical_result_df <- result_df |> dplyr::arrange(gene_id)

# Besag-Clifford results (parallelized)
bc_fs <- list.files(result_dir, pattern = "^bc_res_[0-9]+\\.rds$")
bc_f_names <- sort(bc_fs)
stopifnot(length(bc_f_names) == length(f_names))
bc_res_list <- lapply(paste0(result_dir, bc_f_names), FUN = function(f) readRDS(f))
bc_result_df <- lapply(bc_res_list, FUN = function(r) r$finite_sample_bc_res) |>
  data.table::rbindlist()
bc_result_df$rejected <- NULL
if ("hyp_idx" %in% names(bc_result_df)) bc_result_df$hyp_idx <- NULL
bc_result_df$rejected <- p.adjust(p = bc_result_df$p_value, method = "BH") < 0.1
bc_total_s <- sapply(bc_res_list, FUN = function(r) r$finite_sample_bc_time[["elapsed"]]) |> sum()
seconds_to_period(bc_total_s)
bc_result_df <- bc_result_df |> dplyr::arrange(gene_id)

# load the file for the fast methods
fast_res <- readRDS(paste0(offsite_dir, "results/fast_results.rds"))
asymptotic_result_df <- fast_res$asymptotic_res |> dplyr::arrange(gene_id)
asymptotic_total_s <- fast_res$asymptotic_time[["elapsed"]]
seconds_to_period(asymptotic_total_s)
anytime_result_df <- fast_res$finite_sample_anytime_res |> dplyr::arrange(gene_id)
anytime_total_s <- fast_res$finite_sample_anytime_time[["elapsed"]]
seconds_to_period(anytime_total_s)

# compare the results
all(classical_result_df$gene_id == anytime_result_df$gene_id) &
  all(classical_result_df$gene_id == asymptotic_result_df$gene_id) &
  all(classical_result_df$gene_id == bc_result_df$gene_id)
mean(classical_result_df$rejected)
mean(asymptotic_result_df$rejected)
mean(anytime_result_df$rejected)
mean(bc_result_df$rejected)

# what is the agreement? Among discoveries made by EITHER method A or method B, what percentage are common to both?
compute_discovery_overlap <- function(df_a, df_b) {
  tab <- table(df_a$rejected, df_b$rejected)
  n_union_discoveries <- sum(tab) - tab[1,1]
  n_intersection_discoveries <- tab[2,2]
  n_intersection_discoveries/n_union_discoveries
}
compute_discovery_overlap(classical_result_df, anytime_result_df)
compute_discovery_overlap(classical_result_df, bc_result_df)
compute_discovery_overlap(classical_result_df, asymptotic_result_df)
compute_discovery_overlap(anytime_result_df, bc_result_df)
compute_discovery_overlap(anytime_result_df, asymptotic_result_df)
compute_discovery_overlap(bc_result_df, asymptotic_result_df)
